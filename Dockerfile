...
# ==============================================================================
# Cópia do Código, Artefatos, Dados e Frontend Dist
# ==============================================================================
COPY src/ ./src/
COPY models/ ./models/
COPY data/ ./data/
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=10s --retries=3 \
  CMD python -c "import urllib.request, sys, os; port=os.getenv('PORT', '8000'); sys.exit(0 if urllib.request.urlopen(f'http://localhost:{port}/health', timeout=2).getcode()==200 else 1)"
...
CMD ["sh", "-c", "uvicorn churn_prediction.api.main:app --host 0.0.0.0 --port ${PORT:-8000}"]