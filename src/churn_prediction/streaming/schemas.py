...
class NetworkTelemetryEvent(BaseStreamingEvent):
    ...
    packet_loss_pct: float = Field(
        ..., ge=0.0, le=100.0, description="Porcentagem de pacotes perdidos"
    )
    disconnect_count_last_hour: int = Field(
        default=0, ge=0, description="Quedas de sinal na última hora"
    )
...
class BillingPaymentEvent(BaseStreamingEvent):
    ...
    retry_count: int = Field(default=0, ge=0, description="Número de tentativas de reprocessamento")
...
class CrmInteractionEvent(BaseStreamingEvent):
    ...
    sentiment_score: float = Field(
        ..., ge=-1.0, le=1.0, description="Score de sentimento (-1.0 a +1.0)"
    )
