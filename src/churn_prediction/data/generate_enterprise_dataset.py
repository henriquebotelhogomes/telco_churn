...
def generate_enterprise_dataset(
    ...
    for i in range(1, num_samples + 1):
        ...
        # Contrato
        if tenure > 36:
            contract = random.choices(["Month-to-month", "One year", "Two year"], weights=[0.20, 0.35, 0.45])[0]
        elif tenure > 12:
            contract = random.choices(["Month-to-month", "One year", "Two year"], weights=[0.45, 0.40, 0.15])[0]
        else:
            contract = random.choices(["Month-to-month", "One year", "Two year"], weights=[0.75, 0.20, 0.05])[0]

        phone_service = "Yes" if random.random() < 0.90 else "No"
        multiple_lines = "Yes" if phone_service == "Yes" and random.random() < 0.42 else ("No" if phone_service == "Yes" else "No phone service")
        ...
        if is_chaotic and internet != "No":
            ...
            download_speed = round(plan_speed_mbps * random.uniform(0.85, 1.05), 1) if plan_speed_mbps > 0 else 0.0
            modem_reboots = random.choices([0, 1, 2], weights=[0.60, 0.30, 0.10])[0]
            tech_visits = 0 if random.random() < 0.85 else 1
            nps = random.choices([7, 8, 9, 10, 5, 6], weights=[0.25, 0.30, 0.25, 0.10, 0.05, 0.05])[0]
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Gerador de Dataset Sintético Telco 360 Enterprise")
    parser.add_argument("--num-samples", type=int, default=7043, help="Número de clientes a gerar")
    parser.add_argument("--chaos-ratio", type=float, default=0.12, help="Proporção de clientes com instabilidade de rede")
    parser.add_argument("--output", type=str, default="data/raw/telco_enterprise_customers.csv", help="Caminho de saída")
    ...
    print(f"[SUCCESS] Base Telco 360 Enterprise gerada: {out_path} ({len(df)} clientes, {len(df.columns)} colunas)")
